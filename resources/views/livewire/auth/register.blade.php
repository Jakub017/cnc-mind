        ...
        <div class="w-full max-w-md space-y-6">
            <div class="flex justify-center opacity-50">
                <a href="/" class="group flex items-center gap-3">
                    <span class="text-xl font-semibold text-zinc-800 dark:text-white">CNC Mind</span>
                </a>
            </div>

            <flux:heading class="text-center" size="xl">{{ __('Create an account') }}</flux:heading>

            <div class="space-y-4">
                <flux:button class="w-full" href="{{ route('google.auth.redirect') }}">
                    <x-slot name="icon">
                ...
                <flux:button class="w-full cursor-pointer" href="{{ route('github.auth.redirect') }}">
                    <x-slot name="icon">
                        <svg width="25" height="24" viewBox="0 0 25 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
            ...
            <x-auth-session-status class="text-center text-red-500" :status="session('error')" />
            ...
            <flux:subheading class="text-center">
                ...
                <flux:link wire:navigate href="{{ route('login') }}">{{ __('Sign in') }}</flux:link>
            </flux:subheading>
    ...
    <div class="flex-1 p-4 max-lg:hidden">
        <div class="text-white relative rounded-lg h-full w-full bg-zinc-900 flex flex-col items-start justify-end p-16"
            ...
            <div
                ...
                <flux:heading class="!text-white mb-0 !text-3xl font-bold">{{ __('Welcome to CNC Mind') }}
                </flux:heading>
                <flux:subheading class="!text-zinc-300 !text-lg">
                    {{ __('Elevate your manufacturing with AI-driven insights.') }}</flux:subheading>
            </div>
        </div>
    </div>
</x-layouts.auth>
